...
for log in bro_logs['logs']:
    module_dir = os.path.join('../modules', log['log_type'])
...
for log in bro_logs['logs']:
    config_dir = os.path.join('../modules', log['log_type'], 'config')
    if log.get('pattern', None):
...
for log in bro_logs['logs']:
    ingest_dir = os.path.join('../modules', log['log_type'], 'ingest')
    if log.get('pattern', None):
...
for log in bro_logs['logs']:
    meta_dir = os.path.join('../modules', log['log_type'], '_meta')
    if log.get('pattern', None):
